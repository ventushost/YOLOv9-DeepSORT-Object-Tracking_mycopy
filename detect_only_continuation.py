...
def run(weights, source, data, imgsz, conf_thres, iou_thres, max_det, device, agnostic_nms,
        ...
        tqdm_update, profile, compile_model, copy_local, out_csv=None):
    ...
    # Direkt in Google Drive speichern (nur falls kein out_csv angegeben wurde)
    if out_csv is None:
        ...
        LOG.info(f"Benutzerdefinierter Output-Pfad: {out_csv}")
...
def parse_opt():
    ...
    parser.add_argument('--out-csv', type=str, default=None, help='Optionaler kompletter Pfad zur Zieldatei (CSV). Falls nicht gesetzt, wird automatisch runs/detect/... verwendet.')
